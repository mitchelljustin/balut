...
impl Display for ParserError {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ParserError::ScanError(err) => write!(f, "ScanError({err})"),
            ParseError {
                kind,
                loc,
                derivation,
...
mod patterns {
    ...
    pub macro whitespace() {
        $crate::token::Token::Newline | $crate::token::Token::Dedent | $crate::token::Token::Indent
    }
...
impl Parser {
    ...
    fn add_step(&mut self, rule: &str, step: String) {
        self.derivation.steps.push(format!("{rule:>10}: {step}"));
    }
    ...
    fn phrase(&mut self) -> NodeResult {
        ...
        let mut elements = self.comma_list(|t| matches!(t, patterns::phrase_terminator!()))?;
    ...
    fn comma_list(&mut self, should_terminate: fn(&Token) -> bool) -> Result<Vec<Node>, ErrorKind> {
    ...
    fn array_dict(&mut self) -> NodeResult {
        ...
        if let Sym(":") = self.current() {
            ...
            let node = Node::Literal {
                value: Literal::Dict(HashMap::new()),
            };
            self.returning("array_dict", &node);
            return Ok(node);
        }
        let elements = self.comma_list(|t| matches!(t, Sym("]")))?;
        consume!(self, Sym("]"));
        let n_dict_items = elements
            .iter()
            .filter(|el| matches!(el, Node::Assignment { .. }))
            .count();

        let value = if n_dict_items == elements.len() {
            ...
            for assn in elements {
                ...
                if let Node::Assignment { .. } = *value {
                    return Err(IllegalLiteral {
                        reason: "dict value must be an expr",
                        node: Some(*value),
                    });
                }
            ...
            return Err(IllegalLiteral {
                reason: "illegal dict or array literal",
                node: None,
            });
        };
    ...
    fn path(&mut self) -> NodeResult {
        ...
        while matches!(self.current(), Sym("::")) {
            self.increment();
            let component = self.primary()?;
            if !matches!(component, Node::Ident { .. } | Node::Nomen { .. }) {
                return self.consume_failed("ident or nomen in path");
            }
            components.push(component);
        }
        if components[..components.len() - 1]
            .iter()
            .any(|c| matches!(c, Node::Ident { .. }))
        {}
